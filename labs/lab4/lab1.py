""" Lab 1 : Router Configurations
    ...
    MODIFIED VERSION:
    Dallas Hays & Robert Werthman
    for Lab 4
    3/23/15
    Use with lab4.py
    Lab4 uses the following functions from this python file:
        1. getConfigurations()
        2. printInformation()
        3. isNew()
        4. create_new_version()
        5. create_v1()
    Functions 1, 2, and 3 are different than their originals


    New References:
    http://stackoverflow.com/questions/3430372/how-to-get-full-path-of-
    current-files-directory-in-python
    For absolute paths in python
...
def getConfigurations(connection):
    ...
    for ips in connection:
        ...
        fname = os.path.dirname(os.path.abspath(__file__)) + "/tmpconf/" + ips + "_"
...
def printInformation(connection):
    ...
    for ips in connection:
        ...
        ip_addr = ips
        return ("New config for " + ip_addr + " (" + hostname + ") " +
...
def isNew(connection):
    ...
    for ips in connection:
        fname = os.path.dirname(os.path.abspath(__file__)) + "/tmpconf/" + ips + "_"
        #fname = "/home/netman/tmpconf/" + ips + "_"
        fname2 = os.path.dirname(os.path.abspath(__file__)) + "/routerconfs/" + ips + "_"
